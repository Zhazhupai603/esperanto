...
| map | `index_io::load(paidx)`; `AlignConfig::rna_default()` + `extend.editing_aware=true`; `jlib = gtf.map(gtf::from_gtf…)`; `jkmer = None` (not wired in 1.0.0, see BACKLOG); `l1 = l1_bundle.map(L1Index::open)`; single-pass when L1 present, else 2-pass; then collapsed-alphabet rescue of unmapped reads when `<paidx stem>.cpaidx` exists (survivors appended to raw.bam with MAPQ 0 + `RE:Z:collapsed`, unmapped.fq.gz rewritten) | `<out>/map/raw.bam` + `unmapped.fq.gz` + `align_qc.json` + `align.baln` |
