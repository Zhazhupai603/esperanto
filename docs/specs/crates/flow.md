...
Before `run_pipeline` starts any stage: read `<fasta>.fai`. **With a `species.json` manifest** in the run directory (hybrid runs): the declared baseline decides the expected chr1 length (`grch38` → 248956422, `grcm39` → 195154007, `mm10` → 195471971); human loci are verified at reference build time. **Without a manifest (legacy)**: if a `chr1` line exists, its length must be `== 248956422` (hg38) or `< 10_000_000` (synthetic/test reference), otherwise refuse with `FlowError::SpeciesMismatch{len}`; no `chr1` line passes directly. Same rule as score's internal guardrail (score spec) as a double safeguard — flow guards before burning compute, score guards before scoring. The error message is in English and gives the actual length, the hg38 expected value, and a bundle-matching hint.
...
| map | `index_io::load(paidx)`; `AlignConfig::rna_default()` + `extend.editing_aware=true`; `jlib = gtf.map(gtf::from_gtf…)`; `jkmer = None` (not wired in 1.0.0, see BACKLOG); `l1 = l1_bundle.map(L1Index::open)`; single-pass when L1 present, else 2-pass; then collapsed-alphabet rescue of unmapped reads when `<paidx stem>.cpaidx` exists: artifact reads are screened out (composition/quality gates), survivors are realigned, and every placement is verified against the true four-letter reference — at most 2 non-editing-class mismatches, editing-class (A-to-G/T-to-C) mismatch purity >60% (>80% for reads <=60 bp) at Phred >=30, mismatch-cluster density >=5% of read length, cluster span >=10% of read length not contained in the outer 20% of the read, cluster single-nucleotide share <=60%, ungapped CIGAR, and (paired runs) a mapped mate within 500 kbp in opposite orientation. Only verified reads are appended to raw.bam with MAPQ 0 + `RE:Z:collapsed`; align_qc.json gains `rescued_collapsed` (verified), `rescue_rejected_collapsed`, `rescue_prefiltered_collapsed`; unmapped.fq.gz is rewritten without the verified reads; placements also land in `rescued.bed`, one `chrom\tpos` 0-based row each, for the report's hyperedited-region track | `<out>/map/raw.bam` + `unmapped.fq.gz` + `align_qc.json` + `align.baln` + `rescued.bed` |
...
| filter | `CallFilter::default()` in place on `candidates.bed`: keeps `depth >= 10` AND (`call_score >= 0.9` OR (editing-consistent frequency `>= 0.05` — forward A>G / reverse T>C — with `>= 2` mutation reads)); drops REF=C/G sites and single-read noise before scoring | `candidates.bed` (rewritten in place) |
| score | bed→sites bridge (next section) → `score::pipeline::score_sites_batched(bam=sorted.bam,…)`; BamSites entries parse `--sites` directly (same bridge). Hybrid runs (species.json): sites route per contig owner — human-locus sites score with the human bundle, mouse-contig sites with the mouse bundle declared in the manifest (`ReferenceCheck::TrustedHybrid` skips the chr1 heuristic for both); with no mouse bundle in the manifest, mouse-contig sites reach the VCF as `FILTER=UNSCORED` with VAF/DEPTH kept and `NA` in scores.tsv | `<out>/score/scores.tsv` (`chrom\tpos\tprob`, row order = input order) |
